...
EpotGSSolver::EpotGSSolver( const Geometry &geom )
    : EpotSolver( geom ), _iter_max(1000)
{
...
void EpotGSSolver::gs_process_near_solid( MeshScalarField &epot, const MeshScalarField &rhs,
					 ...
					 uint32_t i, uint32_t j, uint32_t k ) const
{
    ...
    epot(i,j,k) = (1.0/cof) * ( epf - rhs(i,j,k) );
}


void EpotGSSolver::gs_process_pure_vacuum( MeshScalarField &epot, const MeshScalarField &rhs,
					  uint32_t i, uint32_t j, uint32_t k ) const
{
    epot(i,j,k) = (1.0/6.0) * ( epot(i+1,j,k) + epot(i-1,j,k) +
			        epot(i,j+1,k) + epot(i,j-1,k) + 
			        epot(i,j,k+1) + epot(i,j,k-1) - rhs(i,j,k) );
}


void EpotGSSolver::gs_process_neumann( MeshScalarField &epot, const MeshScalarField &rhs,
				      uint32_t boundary, uint32_t i, uint32_t j, uint32_t k ) const
{
    switch( boundary ) {
    case 1:
	// (phi_i - phi_i+1) / h = q_0
	epot(i,j,k) = epot(i+1,j,k) + rhs(i,j,k);
	break;
    case 2:
	// (phi_i-1 - phi_i) / h = q_0
	epot(i,j,k) = epot(i-1,j,k) + rhs(i,j,k);
    ...
    case 3:
	// (phi_j - phi_j+1) / h = q_0
	epot(i,j,k) = epot(i,j+1,k) + rhs(i,j,k);
	break;
    case 4:
	// (phi_j-1 - phi_j) / h = q_0
	epot(i,j,k) = epot(i,j-1,k) + rhs(i,j,k);
    ...
    case 5:
	// (phi_k - phi_k+1) / h = q_0
	epot(i,j,k) = epot(i,j,k+1) + rhs(i,j,k);
	break;
    case 6:
	// (phi_k-1 - phi_k) / h = q_0
	epot(i,j,k) = epot(i,j,k-1) + rhs(i,j,k);
...
void EpotGSSolver::gs_process_node( MeshScalarField &epot, const MeshScalarField &rhs,
				   uint32_t i, uint32_t j, uint32_t k ) const
{
	...
	gs_process_near_solid( epot, rhs, nearsolid_ptr, i, j, k );
    } else if( node_id == SMESH_NODE_ID_PURE_VACUUM ) {
	gs_process_pure_vacuum( epot, rhs, i, j, k );
    } else if( node_id == SMESH_NODE_ID_NEUMANN ) {
	uint32_t boundary = mesh & SMESH_BOUNDARY_NUMBER_MASK;
	gs_process_neumann( epot, rhs, boundary, i, j, k );
    }
}			   


void EpotGSSolver::gs_loop( MeshScalarField &epot, const MeshScalarField &rhs ) const
{
	...
	for( uint32_t j = 0; j < _geom.size(1); j++ ) {
		...
		gs_process_node( epot, rhs, i, j, k );
...
void EpotGSSolver::subsolve( MeshScalarField &epot, const MeshScalarField &scharge ) const
{
    ...
    for( uint32_t iter = 0; iter < _iter_max; iter++ ) {
	gs_loop( epot, scharge );
    }
}
