...
void test1( int argc, char **argv )
{
    ...
    pdb.export_path_manager_data( "export_path_manager1.txt", Ep, 1.0, 2.0, 
				  Vec3D(0,0,0.1),
				  Vec3D(1,0,0),
				  Vec3D(0,1,0) );
...
{
    Geometry geom( MODE_3D, Int3D(26,26,52), Vec3D(0.0,0.0,0.0), 0.002 );
    geom.set_boundary( 1, Bound(BOUND_DIRICHLET, 0.0) );
    geom.set_boundary( 2, Bound(BOUND_DIRICHLET, 0.0) );
    geom.set_boundary( 3, Bound(BOUND_DIRICHLET, 0.0) );
    geom.set_boundary( 4, Bound(BOUND_DIRICHLET, 0.0) );
    ...
    ParticleDataBase3D pdb( geom );
    bool mirror[6] = { true, false, true, false, false, false };
    ...
    pdb.iterate_trajectories( scharge, efield, bfield );
    pdb.export_path_manager_data( "export_path_manager2.txt", Ep, 1.0, 2.0, 
				  Vec3D(0,0,0.1),
				  Vec3D(1,0,0),
				  Vec3D(0,1,0) );
    //pdb.debug_print( cout );
    ...
    /*
      ...
      The test file contains 36 particles with charge of 1 electron
      charge and mass of 2 atomic mass units. Each of the particles is
      carrying 1 A current.

      The particles originate from origin (0,0,0) and have velocity of
      2e6 m/s in z-direction and a flat spread of velocity in
      transverse direction (x and y) from -3e5 m/s to +3e5 m/s.
      The output export is made on z=0.1 m plane.
